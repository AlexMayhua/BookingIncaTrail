...
const FootprintIcon = ({ isActive }) => (
    ...
    >
        ...
        <g className={isActive ? 'animate-step-1' : ''} style={{ transformOrigin: '30px 32px' }}>
        ...
        <g className={isActive ? 'animate-step-2' : ''} style={{ transformOrigin: '10px 20px' }}>
        ...
        <g className={isActive ? 'animate-step-3' : ''} style={{ transformOrigin: '30px 8px' }}>
...
export default function AccordionFromHtml({ htmlContent }) {
    ...
    const toggleAccordion = (index) => {
        if (activeIndex === index) {
            ...
            if (firstH3Ref.current) {
                const elementTop = firstH3Ref.current.getBoundingClientRect().top + window.scrollY;
                const offset = 175 - (index * 85.6);
                window.scrollTo({
    ...
    return (
        <>
            {/* Animaciones CSS - pasos de ABAJO hacia ARRIBA */}
            <style jsx global>{`
                ...
                @keyframes glowPulse {
                    0%, 100% {
                        box-shadow: 0 0 10px rgba(251, 184, 0, 0.6), 0 0 25px rgba(251, 184, 0, 0.3);
                    }
                    50% {
                        box-shadow: 0 0 20px rgba(251, 184, 0, 0.8), 0 0 40px rgba(251, 184, 0, 0.5);
                    }
                }
                ...
                .accordion-item-modern:hover .accordion-header-modern {
                    background: linear-gradient(135deg, rgba(251, 184, 0, 0.12) 0%, rgba(251, 184, 0, 0.06) 100%);
            ...
            <div className="relative py-4">
                ...
                <div
                    ...
                    style={{
                        zIndex: 0,
                        background: 'linear-gradient(180deg, #fbb800 0%, #1e5631 50%, #fbb800 100%)',
                        opacity: 0.5
                    }}
                />
                ...
                {blocks.map((block, index) => {
                    ...
                    return (
                        <div
                            key={index}
                            ref={index === 0 ? firstH3Ref : null}
                            className="accordion-item-modern relative pl-16 pb-5 last:pb-0"
                        >
                            {/* Círculo indicador con icono de huellas */}
                            <div
                                className={`absolute left-0 top-3 w-14 h-14 rounded-full flex items-center justify-center transition-all duration-500 ${isActive
                            ...
                            <div
                                ...
                                className={`accordion-header-modern cursor-pointer rounded-xl p-4 transition-all duration-300 border-2 ${isActive
                            ...
                            >
                                <div className="flex items-center justify-between gap-3">
                                    <h3
                                        className={`text-lg font-bold transition-colors duration-300 leading-tight ${isActive ? 'text-primary' : 'text-stone-700'
                                            }`}
                                        dangerouslySetInnerHTML={{ __html: title }}
                                    />

                                    {/* Flecha indicadora */}
                                    <div className={`flex-shrink-0 w-9 h-9 rounded-full flex items-center justify-center transition-all duration-400 ${isActive ? 'bg-primary rotate-180 scale-110' : 'bg-stone-200 hover:bg-stone-300'
                                        }`}>
                                        <svg
                                            className={`w-5 h-5 transition-all duration-300 ${isActive ? 'text-secondary' : 'text-stone-500'
                                                }`}
                                            fill="none"
                                            viewBox="0 0 24 24"
                                            stroke="currentColor"
